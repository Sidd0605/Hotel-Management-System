...
class booking:
    def __init__(self, root):
        ...
        btnsearch = Button(table_frame, text="Search", command=self.search, font=("time new roman", 12, "bold"), bg="gold", fg="black", width=8)
        btnsearch.grid(row=0, column=3, padx=2)

        btnall = Button(table_frame, text="View all", command=self.fetch_data, font=("time new roman", 12, "bold"), bg="gold", fg="black", width=8)
        btnall.grid(row=0, column=4, padx=2)
        ...
        self.booking_details_table = ttk.Treeview(details_table, column=("Customer ID", "Room Number", "Check In", "Check Out", "No of days", "Price", "Total Price"),
    ...
    def add_data(self):
        if self.var_customer_id.get() == "" or self.var_roomno.get() == "":
            ...
            try:
                # Extract price from the selected value
                price_value = self.var_price.get().split('=')[1]  # This gets the numeric part of the price
                ...
                my_cursor.execute("INSERT INTO booking VALUES (%s, %s, %s, %s, %s, %s, %s)", (
                    ...
                    self.var_end_date.get(),
                    self.var_days.get(),
                    price_value,  # Use the numeric price here
                    self.var_total_price.get()
                ))
                conn.commit()
                self.fetch_data()
    ...
    def delete(self):
        if self.var_customer_id.get() == "":
            ...
            try:
                ...
                my_cursor.execute(sql, (self.var_customer_id.get(),))
                conn.commit()
                conn.close()
                messagebox.showinfo("Success", "Booking has been deleted", parent=self.root)
                self.fetch_data()
            except Exception as e:
                messagebox.showerror("Error", f"Error due to: {str(e)}", parent=self.root)
    ...
    def total(self):
        ...
        try:
            ...
            self.var_days.set(days)

            if days < 0:
                messagebox.showerror("Error", "Check-out date must be after check-in date.", parent=self.root)
                return

            price_map = {"Normal=2000": 2000, "Deluxe=5000": 5000, "Adjoint=7000": 7000, "Luxury=10000": 10000}
            totalprice = days * price_map.get(self.var_price.get(), 0)
            self.var_total_price.set(totalprice)
        except ValueError:
    ...
    def fetch_data(self):
        ...
        if len(rows) != 0:
            self.booking_details_table.delete(*self.booking_details_table.get_children())
            for row in rows:
                self.booking_details_table.insert("", END, values=row)
            conn.commit()
        conn.close()
